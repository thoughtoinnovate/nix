...
write_sh_script() {
  local destination="$1"
  printf '#!%s\n' "$(command -v sh)" >"$destination"
}

write_sh_script "$ROOT/installer.sh"
...
write_sh_script "$ROOT/source/Fixture CLI.app/Contents/MacOS/fixture-cli"
...
EOF
write_sh_script "$ROOT/bin/sha256sum"
cat >> "$ROOT/bin/sha256sum" <<'EOF'
printf 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  %s\n' "$1"
EOF
write_sh_script "$ROOT/bin/hdiutil"
...
test "$1" = detach
EOF
write_sh_script "$ROOT/bin/ditto"
cat >> "$ROOT/bin/ditto" <<'EOF'
cp -R "$1" "$2"
EOF
write_sh_script "$ROOT/bin/codesign"
