...
for trust_file in "$installer" "$flake" "$launcher" "$setup"; do
  [[ -n "$trust_file" ]] || continue
  grep -Eq 'substituters = (\[ )?"?https://cache\.nixos\.org/' "$trust_file"
  grep -Fq "$official_key" "$trust_file"
  grep -Fq 'require-sigs = true' "$trust_file"
  grep -Fq 'sandbox = true' "$trust_file"
  if grep -Fq 'extra-substituters' "$trust_file"; then
    printf 'third-party cache extension found in %s\n' "$trust_file" >&2
    exit 1
  fi
done
...
if [[ -n "$lock" ]]; then
  jq -e '
    [.nodes[] | select(.locked?)] | all(
      .locked.narHash != null and
      (
        [.locked.owner, .locked.repo] as $source |
        [["NixOS","nixpkgs"],
         ["nix-darwin","nix-darwin"], ["numtide","flake-utils"],
         ["nix-systems","default"]] | any(. == $source)
      )
    )
  ' "$lock" >/dev/null || {
    printf 'flake.lock contains an unpinned or unreviewed public input\n' >&2
    exit 1
  }
fi
...
grep -Fq 'PROFILE_SHELLS_JSON="$(jq -c '\''.shells'\'' <<<"$profile_json")"' "$installer" || {
  printf 'generated activation does not serialize the resolved shell list\n' >&2
  exit 1
}
grep -Fq 'declaredShells = builtins.fromJSON' "$installer" || {
  printf 'generated activation does not consume the resolved shell list\n' >&2
  exit 1
}
grep -Fq '++ map (shell: pkgs.shellPackages.\${shell}) declaredShells;' "$installer" || {
  printf 'generated package environment omits resolved shell packages\n' >&2
  exit 1
}
grep -Fq 'paths = activationPackages;' "$installer" || {
  printf 'generated package environment does not install the full activation package set\n' >&2
  exit 1
}
