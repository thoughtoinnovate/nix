...
let
  ...
  nativeProvider = prev.writeShellApplication {
    name = "home-weave-native-provider";
    ...
    text = ''
      exec ${final.bash}/bin/bash ${../lib/native-provider.sh} "$@"
    '';
  };
  ...
  homeWeaveCli = prev.writeShellApplication {
    ...
    text = ''
      ...
      export HOME_WEAVE_NATIVE_PROVIDER="''${HOME_WEAVE_NATIVE_PROVIDER:-${nativeProvider}/bin/home-weave-native-provider}"
      ...
      export HOME_WEAVE_CONFIG_SCHEMA="''${HOME_WEAVE_CONFIG_SCHEMA:-${../schemas/home-weave-v4.schema.json}}"
...
in
{
  ...
  home-weave-native-provider = nativeProvider;
